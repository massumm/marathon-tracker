...
class _MyPageMapScreenState extends State<MyPageMapScreen> {
  ...
  String get _dateTitle =>
      TrackedRoute.parseDateFromFileName(widget.filePath.split('/').last);
  ...
  Future<void> _loadRoute() async {
    try {
      final url =
          await FirebaseService.instance.getDownloadUrl(widget.filePath);
      final response = await http.get(Uri.parse(url));
      final json = jsonDecode(response.body) as Map<String, dynamic>;
      final route = TrackedRoute.fromJson(json, widget.filePath);
      setState(() {
        _route = route;
        _isLoaded = true;
      });
          ...
          ? Stack(
              children: [
                GoogleMap(
                  ...
                  polylines: {
                    Polyline(
                      polylineId: PolylineId(widget.filePath),
                      points: _route!.route,
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            _divider(),
                            _statColumn(
                                Icons.timer, _route!.time, '時間'),
                            _divider(),
                            _statColumn(
                                Icons.speed, _route!.pace, 'ペース'),
                          ],
                        ),
                      ),
                    ),
  ...
  Widget _divider() => Container(
        height: 36, width: 1, color: const Color(0xFFE5E7EB));
}
